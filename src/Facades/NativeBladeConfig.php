 ...
 * @method static void globalShortcuts(array $shortcuts) Desktop-only system-wide keyboard shortcuts, as an `accelerator => name` map. Each press fires the `nb:shortcut` Livewire event with that name. Requires `Plugin::GLOBAL_SHORTCUT`; no-op on mobile.
 ...
 * @method static static admob(string $androidAppId, ?string $iosAppId = null, ?string $trackingDescription = null) Enable AdMob (rewarded + interstitial). Writes the app id to the Android manifest and iOS Info.plist, and keeps the AD_ID permission. Needs Plugin::ADMOB.

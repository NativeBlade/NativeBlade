...
class ShellConfig
{
    ...
    /**
     * Enable universal links (iOS) / app links (Android): verified https deep
     * links that open the app directly.
     *
     * `$domains` drives the native setup (Associated Domains entitlement on
     * iOS, autoVerify intent filters on Android) generated by
     * `nativeblade:config`. The optional `$handler` receives each incoming URL
     * and should return a `NativeResponse` (e.g. `NativeBlade::navigate(...)`)
     * to route it; it runs for both cold-start and while-running links.
     *
     * @param  string[]  $domains  Bare hosts, e.g. ['myapp.com', 'www.myapp.com'].
     * @param  Closure(string): mixed|null  $handler
     */
    public function deepLinks(array $domains, ?Closure $handler = null): static
    {
        static::$appConfigs['deepLinks'] = ['domains' => array_values($domains)];
        if ($handler) {
            \NativeBlade\Plugins\DeepLinkRegistry::setOnLink($handler);
        }
        return $this;
    }

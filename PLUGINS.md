...
| `Plugin::TASK_MANAGER` | Background task courier (Rust): periodic `fetch`/`post` declared via `NativeBladeConfig::backgroundTasks()`, consumed via `NativeBlade::getTask()` — see [TASKS.md](TASKS.md) |
| `Plugin::SENSORS` | Device sensors (accelerometer, gyroscope, magnetometer, barometer, light) via `NativeBlade::sensors()` — see [SENSORS.md](SENSORS.md) |

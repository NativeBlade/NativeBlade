---
title: "MCP"
description: "The NativeBlade MCP server."
---
...
Returns the list of documentation pages with their topic and a short summary.
Each name is a path under the docs directory. The agent uses this to figure out
which page to read for a given question.

```json
{
    "docs": [
        {"name": "core/animations.md", "title": "Animations", "summary": "..."},
        {"name": "core/plugins.md",    "title": "Plugins", "summary": "..."},
        ...
    ]
}
```
...
Returns the full Markdown content of one page. Pass the path from list_docs. Only files inside the framework docs directory are readable, path traversal is blocked.

```json
{ "name": "mobile/media.md" }
```

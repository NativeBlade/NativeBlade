---
title: "Portal"
description: "The NativeBlade Portal."
---
...
1. On your machine, `nativeblade:dev --platform=portal` bundles your Laravel app and serves it from a local dev server.
2. The Portal app on the device (it ships with php-wasm, the Livewire runtime, and the native bridges) loads that bundle from the URL you paste or scan.
3. It runs the bundle and polls the dev server for changes, so your edits reload live on the device.

---
title: "Cache"
description: "Cache and persistent key-value state."
---
...
- **Large blobs** (more than a few MB) are not a great fit. The cache row stores `value` as text and the whole SQLite file is checkpointed to IndexedDB after writes, bigger file means slower checkpoint. For binary blobs prefer `Storage::disk('nativeblade')` (see [Filesystem](/core/filesystem/)).

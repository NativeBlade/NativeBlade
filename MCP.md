...
Returns the canonical NativeBlade pattern for a specific use case: component-controller, form-validation, global-state, push-handler, deep-link, biometric-flow, multiple-http-pool, repository-vs-eloquent, http-client, file-organization, debugging, anti-patterns. Call with no arguments to list every available recipe; call with `use_case=<name>` for the full text plus example code.
